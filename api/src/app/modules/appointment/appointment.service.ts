...
//     const docFee = parseFloat(isDoctorExist.price || "60");
//     const vat = (15 / 100) * (docFee);
...
//       appointmentId: new mongoose.Types.ObjectId(appointment[0]._id as any),
//       bookingFee: 10,
...
//     await Appointment.findByIdAndUpdate(appointment[0]._id, {
  ...
  try {
    ...
    if (Types.ObjectId.isValid(id)) {
      ...
      result = await Appointment.findOne({ trackingId: objectId })
        .populate(
          "doctorId",
          "firstName lastName designation college degree img"
        )
        .populate(
          "patientId",
          "firstName lastName address city country state img"
        );
    } else {
...
const getPatientAppointmentById = async (user: any): Promise<any[] | null> => {
  const { userId } = user;
  const result = await Appointment.find({ patientId: user.patientId }).populate(
    "doctorId"
  );
  return result;
};

const getPaymentInfoViaAppintmentId = async (
  id: string
): Promise<any | null> => {
  try {
    if (!id) {
    ...
    // Đảm bảo ID được chuyển đổi thành ObjectId
    let objectId;
    try {
    ...
    // Truy vấn để kiểm tra nếu payment tồn tại
    const paymentCheck = await Payment.findOne({ appointmentId: objectId });
    console.log("Payment check:", paymentCheck);
    if (!paymentCheck) {
      throw new Error("No payment record found for appointmentId");
    }
    console.log("Payment record found:", paymentCheck);

    // Thực hiện populate các trường liên quan
    const result = await Appointment.findOne({ _id: objectId })
      .populate({
        path: "paymentId",
        select:
          "paymentMethod paymentType DoctorFee bookingFee vat totalAmount",
      })
      .populate({
        path: "patientId",
        select: "firstName lastName email mobile address city country",
      })
      .populate({
        path: "doctorId",
        select: "firstName lastName email mobile address city country",
    ...
    if (!result) {
      throw new Error("No payment found after populate for appointmentId");
    }

    console.log("Payment Info:", result);
    return result;
  } catch (error: any) {
    console.error("Error in getPaymentInfoViaAppintmentId:", error.message);
    return null;
  }
};
...
const getPatientPaymentInfo = async (user: any): Promise<any[]> => {
  ...
  const result = await Appointment.find({ patientId: isUserExist._id })
    ...
    .populate({
      path: "paymentId",
      select: "totalAmount paymentMethod paymentType",
    });
...
// console.log("isUserExist",isUserExist)
  ...
  const appointmentIdDoctor = await Appointment.find({doctorId: isUserExist._id});
  if (!appointmentIdDoctor || appointmentIdDoctor.length === 0) {
    throw new ApiError(httpStatus.NOT_FOUND, "Doctor Account is not found !!");
  }
  // console.log("appointmentIdDoctor: ",appointmentIdDoctor)
  const appoitmentIds = appointmentIdDoctor.map((appointmentId) => appointmentId._id)
  // {appointmentId: isUserExist._id }
  const result = await Payment.find({appointmentId: appoitmentIds }).populate({
    path: "appointmentId",
    populate: {
      path: "patientId",
...
const getDoctorAppointmentsById = async (
  ...
  let andCondition: {
    ...
    scheduleDate?: { $gte?: string; $lt?: string };
  } = { doctorId: doctorId };
...
const getDoctorPatients = async (user: any): Promise<any[]> => {
  ...
  const appointments = await Appointment.find({ doctorId: doctorId }).distinct(
    "patientId"
  );
...
const updateAppointmentByDoctor = async (
  ...
  const result = await Appointment.findByIdAndUpdate(payload.id, payload, {
    new: true,
  });
  return result;
};
